...
void display(void){
	...
	glLoadIdentity();
	gluLookAt(eye_pos[0],eye_pos[1],eye_pos[2],0.f,0.f,-1.f,0.f,1.f,0.f);
	glPushMatrix();
	glRotatef(angles[0],1.f,0.f,0.f);
	glRotatef(angles[1],0.f,1.f,0.f);
	glRotatef(angles[2],0.f,0.f,1.f);
...
void onMotion(GLint x, GLint y){
	if (g_bLbutton){
		angles[1] += 0.03*(x - g_lastX);
		angles[0] += 0.03*(y - g_lastY);
	}
}
